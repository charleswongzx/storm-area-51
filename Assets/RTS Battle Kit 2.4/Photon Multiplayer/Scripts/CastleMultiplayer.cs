...
public class CastleMultiplayer : Photon.PunBehaviour, IPunObservable {
    ...
    public AudioClip castleExplosion;
	...
	public float masterCastleStrength;
	public float clientCastleStrength;
    ...
    void Start()
    {
        InvokeRepeating("tickCooldown", 0f, 1f);  //1s delay, repeat every 1s
    }
	...
	void UpdateUnitButtons(){
		for(int i = 0; i < troops.Length; i++){
			if(troops[i].troopCosts <= gold && troops[i].currentCooldownTime >= troops[i].maxCooldownTime){
            ...
            /*if (troops[i].currentCooldownTime < troops[i].maxCooldownTime)
            {
                //troops[i].button.GetComponent<Image>().fillAmount = troops[i].currentCooldownTime / troops[i].maxCooldownTime;
            }*/
        }
	}
	...
	void TrySpawn(){
		...
		if(hit.collider != null && hit.collider.gameObject.CompareTag("Battle ground") && CanSpawn(hit.point.x)){
            Debug.Log("Spawning..." + troops[selectedUnit].currentCooldownTime + " " + troops[selectedUnit].maxCooldownTime);
            ...
            // check if unit is cooling down
            if (troops[selectedUnit].currentCooldownTime < troops[selectedUnit].maxCooldownTime)
            {
                StartCoroutine(CooldownWarning());
                return;
            }
            ...
            troops[selectedUnit].currentCooldownTime = 0;
            UpdateUnitButtons();
		}
    ...
    void tickCooldown()
    {
            for (int i = 0; i < troops.Length; i++)
            {
                if (i == 0) Debug.Log(troops[i].currentCooldownTime);
                if (troops[i].currentCooldownTime < troops[i].maxCooldownTime)
                {
                    troops[i].currentCooldownTime++;
                }
            }
    }
    ...
    //show a warning if unit still coolding down
    IEnumerator CooldownWarning()
    {
        if (!gameUI.cooldownWarning.activeSelf)
        {
            gameUI.cooldownWarning.SetActive(true);

            yield return new WaitForSeconds(1);
            gameUI.cooldownWarning.SetActive(false);
        }
    }
	...
	public void DestroyUnit(GameObject unit, string type, GameObject dieEffect){
        AudioSource.PlayClipAtPoint(unitDeath, transform.position);
	...
	void CheckCastle(int castlePartIndex){
        ...
        {
            AudioSource.PlayClipAtPoint(castleExplosion, transform.position);
	...
	//end a battle
	public void EndBattle(bool masterWon){
